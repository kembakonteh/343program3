#include <iostream>

//---------------------------------------------------------------------------
// lab3.cpp
//---------------------------------------------------------------------------
// This code tests the basic functionality of the classes to perform
// Dijkstra's algorihms and depth-first search for CSS 343 Lab 3.
// It is not meant to exhaustively test the class.
//
// Assumptions:
//   -- students can follow the directions to interface with this file
//   -- text files "data31.txt" and "data32.txt" are formatted as described
//   -- Data file data3uwb provides an additional data set for part 1;
//      it must be edited, as it starts with a description how to use it
//---------------------------------------------------------------------------

#include <iostream>
#include <fstream>
#include "GraphL.h"
#include "GraphM.h"
using namespace std;

int main() {

    // part 1
    cout << "Testing part 1 with my data read in as data31K.txt  " << endl;
    ifstream infile1("data31K.txt");
    if (!infile1) {
        cout << "File could not be opened." << endl;
        return 1;
    }

    //for each graph, find the shortest path from every node to all other nodes
    for (;;) {
        GraphM myGraph;
        myGraph.buildGraph(infile1);
        if (infile1.eof())
            break;
        myGraph.findShortestPath();
        myGraph.displayAll();              // display shortest distance, path to cout
        myGraph.display(2, 1);              // display path from node 3 to 1 to cout
        myGraph.display(1, 5);
        myGraph.display(1, 3);
        myGraph.display(1,2);
        myGraph.display(1,4);
        myGraph.display(2,4);

        // part 2
        cout << " Testing part 2 with my data read in as data32K.txt " << endl;
        ifstream infile2("data32K.txt");
        if (!infile2) {
            cout << "File could not be opened." << endl;
            return 1;
        }

        //for each graph, find the depth-first search ordering
        for (;;) {
            GraphL myGraph;
            myGraph.buildGraph(infile2);
            if (infile2.eof())
                break;
            myGraph.displayGraph();
            myGraph.depthFirstSearch();    // find and display depth-first ordering to cout

            cout << " " << endl;



        }

        cout << endl;
        return 0;
    }

}


